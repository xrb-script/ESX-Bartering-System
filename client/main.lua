...
local contractPed = nil; local contractPedBlip = nil; local localCurrentContractPedLocationIndex = -1;
local shopPeds = {}

-- [[ Shop Ped Functions ]]
local function spawnShopPeds() if not Config.Shops or #Config.Shops == 0 then return end; for i, shopData in ipairs(Config.Shops) do if shopData.id and shopData.ped_model and shopData.ped_coords then RequestModel(shopData.ped_model); local attempts = 0; while not HasModelLoaded(shopData.ped_model) and attempts < 50 do Wait(100); attempts = attempts + 1 end; if HasModelLoaded(shopData.ped_model) then local ped = CreatePed(4, shopData.ped_model, shopData.ped_coords.x, shopData.ped_coords.y, shopData.ped_coords.z - 1.0, shopData.ped_heading or 0.0, true, true); FreezeEntityPosition(ped, true); SetEntityInvincible(ped, true); SetBlockingOfNonTemporaryEvents(ped, true); if shopData.ped_scenario then TaskStartScenarioInPlace(ped, shopData.ped_scenario, 0, true) end; SetModelAsNoLongerNeeded(shopData.ped_model); shopPeds[shopData.id] = ped; exports.ox_target:addLocalEntity(ped, {{ name = ('open_shop_%s'):format(shopData.id), icon = 'fas fa-store', label = Config.Messages['open_shop_target_label']:format(shopData.label), distance = 1.5, canInteract = function() return not isPlayerDead end, onSelect = function() TriggerServerEvent('xrb-bartering:requestShopAccess', shopData.id) end }}) else print(('[xrb-Bartering] [ERROR] Shop ped model "%s" for shop "%s" failed to load!'):format(shopData.ped_model, shopData.id)) end else print(('[xrb-Bartering] [ERROR] Invalid config for shop at index %d'):format(i)) end end end

-- [[ Contract Ped Functions ]]
local function spawnOrUpdateContractPed(locationIndex) local location = Config.PedLocations[locationIndex]; if not location then print(('[xrb-Bartering] [ERROR] Contract location index %s not found.'):format(locationIndex)); return end; if contractPed then exports.ox_target:removeLocalEntity(contractPed); if DoesEntityExist(contractPed) then DeleteEntity(contractPed) end; contractPed = nil end; if contractPedBlip and DoesBlipExist(contractPedBlip) then RemoveBlip(contractPedBlip); contractPedBlip = nil end; RequestModel(Config.PedModel); local attempts = 0; while not HasModelLoaded(Config.PedModel) and attempts < 50 do Wait(100); attempts = attempts + 1 end; if not HasModelLoaded(Config.PedModel) then print(('[xrb-Bartering] [CRITICAL ERROR] Contract ped model "%s" failed load!'):format(Config.PedModel)); return end; contractPed = CreatePed(4, Config.PedModel, location.x, location.y, location.z - 1.0, GetRandomFloatInRange(0.0, 360.0), true, true); FreezeEntityPosition(contractPed, true); SetEntityInvincible(contractPed, true); SetBlockingOfNonTemporaryEvents(contractPed, true); TaskStartScenarioInPlace(contractPed, 'WORLD_HUMAN_STAND_IMPATIENT', 0, true); SetModelAsNoLongerNeeded(Config.PedModel); contractPedBlip = AddBlipForEntity(contractPed); SetBlipSprite(contractPedBlip, 1); SetBlipScale(contractPedBlip, 0.8); SetBlipColour(contractPedBlip, 5); SetBlipAsShortRange(contractPedBlip, true); BeginTextCommandSetBlipName("STRING"); AddTextComponentString(Config.Messages['bartering_ped_label']); EndTextCommandSetBlipName(contractPedBlip); exports.ox_target:addLocalEntity(contractPed, { { name = 'barter_start', icon = 'fas fa-handshake', label = Config.Messages['start_contract'], canInteract = function() return not activeContract and not isPlayerDead end, onSelect = function() TriggerServerEvent('xrb-bartering:requestContract') end }, { name = 'barter_submit', icon = 'fas fa-box-open', label = Config.Messages['submit_items'], canInteract = function() return activeContract ~= nil and not isPlayerDead end, onSelect = function() if activeContract then TriggerServerEvent('xrb-bartering:submitItems') else exports.ox_lib:notify({ title = 'Error', description = Config.Messages['no_active_contract'], type = 'error' }) end end }, { name = 'barter_cancel', icon = 'fas fa-ban', label = Config.Messages['cancel_contract'], canInteract = function() return activeContract ~= nil and not isPlayerDead end, onSelect = function() if activeContract then TriggerServerEvent('xrb-bartering:requestCancelContract') else exports.ox_lib:notify({ title = 'Error', description = Config.Messages['no_active_contract'], type = 'error' }) end end } }); localCurrentContractPedLocationIndex = locationIndex end

-- [[ Initialize Script & Player Data ]]
Citizen.CreateThread(function() while ESX == nil do ESX = exports["es_extended"]:getSharedObject(); Citizen.Wait(100) end; while not ESX.IsPlayerLoaded() do Citizen.Wait(10) end; PlayerData = ESX.GetPlayerData(); Citizen.Wait(2500); TriggerServerEvent('xrb-bartering:requestInitialState'); spawnShopPeds(); StartDeathCheckThread(); end)

-- [[ Shop UI ]]
RegisterNetEvent('xrb-bartering:openOxLibShop', function(shopLabel, shopStockData, shopId) if not ESX then return end; if not shopStockData or #shopStockData == 0 then exports.ox_lib:notify({ title = shopLabel, description = Config.Messages['shop_empty'], type = 'inform' }); return end; local shopOptions = {}; for i, itemData in ipairs(shopStockData) do if itemData and itemData.name and itemData.stock ~= nil and itemData.price ~= nil and itemData.currency then local itemLabel = exports.ox_inventory:Items(itemData.name)?.label or ESX.GetItemLabel(itemData.name) or itemData.name; local currencyLabel = itemData.currency:upper(); table.insert(shopOptions, { title = string.format("%s (Stock: %d)", itemLabel, itemData.stock), description = string.format(Config.Messages['shop_item_description'], itemData.price, currencyLabel), icon = 'fas fa-box', disabled = (itemData.stock <= 0), onSelect = function() if itemData.stock > 0 then TriggerServerEvent('xrb-bartering:purchaseShopItem', shopId, itemData.name, itemData.price, itemData.currency) else exports.ox_lib:notify({ title = 'Error', description = Config.Messages['shop_purchase_failed_stock'], type = 'error' }) end end }) else print(('[xrb-Bartering] [ERROR] Invalid shop item data index %s for shop %s'):format(i, shopId)) end end; if #shopOptions > 0 then lib.registerContext({ id = ('barter_shop_%s'):format(shopId), title = Config.Messages['shop_title']:format(shopLabel), options = shopOptions }); lib.showContext(('barter_shop_%s'):format(shopId)) else exports.ox_lib:notify({ title = shopLabel, description = Config.Messages['shop_empty'], type = 'inform' }) end end)
...
RegisterNetEvent('xrb-bartering:showStatusMenu', function(statusData)
    ...
    table.insert(options, { title = "Current Points: " .. points, description = "Your current bartering points balance.", icon = 'fas fa-coins', disabled = true })
    ...
    if contract and contract.contractData then
        ...
        table.insert(options, { title = "Contract: " .. (contract.contractData.name or "Unknown"), description = string.format("Time Remaining: %s:%02d", minutes, seconds), icon = 'fas fa-file-contract', disabled = true })
        table.insert(options, { title = "Points Reward: " .. tostring(pointsReward), description = "Points awarded upon completion.", icon = 'fas fa-star', disabled = true })

        if contract.contractData.required_items and #contract.contractData.required_items > 0 then
             table.insert(options, { title = "Required Items:", description = "--------------------", disabled = true})
            for _, req in ipairs(contract.contractData.required_items) do local itemLabel = exports.ox_inventory:Items(req.item)?.label or ESX.GetItemLabel(req.item) or req.item; table.insert(options, { title = string.format("- %sx %s", req.count, itemLabel), description = "Item needed for the contract.", icon = 'fas fa-box', disabled = true }) end
        else table.insert(options, { title = "No specific items required.", icon = 'fas fa-check', disabled = true}) end
    else table.insert(options, { title = Config.Messages['bartering_command_no_contract'], icon = 'fas fa-times-circle', disabled = true }) end

    lib.registerContext({ id = 'barter_status_menu', title = Config.Messages['bartering_command_info_title'], options = options })
    lib.showContext('barter_status_menu')
end)

-- [[ Player Death Check (Manual Loop for ESX) ]]
function StartDeathCheckThread() Citizen.CreateThread(function() while true do Citizen.Wait(1000); local playerPed = PlayerPedId(); local currentlyDead = IsEntityDead(playerPed); if currentlyDead and not isPlayerDead then isPlayerDead = true; if activeContract then TriggerServerEvent('xrb-bartering:server:playerDied') end elseif not currentlyDead and isPlayerDead then isPlayerDead = false end end end) end
